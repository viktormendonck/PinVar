	...
	[
		...
		[
			...
			[
				...
				[
					SNew(STextBlock).Text(FText::FromString(TEXT("Add BP Variable")))
				]
			]
			...
			[
				...
				[
					SNew(STextBlock).Text(FText::FromString(TEXT("Remove BP Variable")))
				]
			]
		]
...
void SPinVarPanel::Rebuild()
{
	...
	if (Grouped.Num() == 0)
	{
		...
		[
			SNew(STextBlock)
			.Text(FText::FromString(TEXT("No pinned variables yet. Use “Add BP Variable” to stage entries.")))
		];
		return;
	}
...
void SPinVarPanel::ShowRemoveStagedDialog()
{
...

	{
		...
		Subsystem->GetAllStaged(Triples);
		...
		for (const auto& T : Triples)
		{
			...
			if (UClass* C = FindObject<UClass>(nullptr, *ClassStr))
			{
				ClassStr = C->GetName();
			}
				...
				.OnClicked_Lambda([this, Dialog, State, Subsystem]() -> FReply
				{
					if (State->Selected.IsValid())
					{
						Subsystem->UnstagePinVariable(State->Selected->ClassName,
							State->Selected->VarName, State->Selected->GroupName);
						Refresh();
					}
		...
		[
			...
			[
				...
				.OnClicked_Lambda([this, Dialog, State]()
				{
					...
					{
						// Ensure we have a usable class to bind property views
						if (!State->BP->GeneratedClass)
						{
						...
						if (State->BP->GeneratedClass)
							ClassName = State->BP->GeneratedClass->GetFName();
						else if (State->BP->SkeletonGeneratedClass)
							ClassName = State->BP->SkeletonGeneratedClass->GetFName();

						if (!ClassName.IsNone() && GEditor)
						{
							if (UPinVarSubsystem* Subsystem = GEditor->GetEditorSubsystem<UPinVarSubsystem>())
							{
								// Allow comma/pipe; normalize to commas and stage each token
								FString GroupStr = State->GroupName;
								GroupStr.ReplaceCharInline(TEXT('|'), TEXT(','));

								TArray<FString> Groups;
								GroupStr.ParseIntoArray(Groups, TEXT(","), true);
