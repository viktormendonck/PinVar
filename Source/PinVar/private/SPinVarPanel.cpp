...
void SPinVarPanel::BuildComponentOptions(UBlueprint* BP, UClass* Class, TArray<TSharedPtr<FCompOption>>& Out)
{
    ...
    for (UClass* C = Class; C; C = C->GetSuperClass())
    {
        if (UObject* CDO = C->GetDefaultObject(true))
        {
            ...
            for (UObject* O : Subs)
            {
                ...
                TSharedRef<FCompOption> Opt = MakeShared<FCompOption>();
                Opt->Label = TmplName;
    ...
    Out.StableSort([](const TSharedPtr<FCompOption>& A,
                      const TSharedPtr<FCompOption>& B)
    {
...
void SPinVarPanel::Construct(const FArguments& InArgs)
{
           ...
           .OnClicked_Lambda([this]()
    {
        ...
        + SHorizontalBox::Slot().AutoWidth().Padding(6.f, 0.f)[SNew(SButton).ButtonStyle(FAppStyle::Get(), "FlatButton").OnClicked(this, &SPinVarPanel::OnAddBlueprintVariableClicked)[SNew(STextBlock).Text(FText::FromString(TEXT("Add Variable")))]]]

     // Scrollable list
     + SVerticalBox::Slot().FillHeight(1.f)[SNew(SScrollBox) + SScrollBox::Slot()[SAssignNew(RootBox, SVerticalBox)]]];
...
bool SPinVarPanel::IsEditableProperty(const FProperty* P)
{
    ...
    const bool bIsDelegate = P->IsA(FMulticastDelegateProperty::StaticClass()) || P->IsA(FDelegateProperty::StaticClass());
...
void SPinVarPanel::GetAllGroups(TSharedRef<FState> S)
{
    ...
    if (GEditor)
    {
        if (UPinVarSubsystem* Subsystem = GEditor->GetEditorSubsystem<UPinVarSubsystem>())
        {
            ...
            for (const auto& Pair : Subsystem->PinnedGroups)
            {
                for (const FPinnedVariable& E : Pair.Value)
                {
                    if (!E.GroupName.IsNone())
...
void SPinVarPanel::GatherPinnedProperties()
{
    ...
    for (const TPair<FName, TArray<FPinnedVariable>>& Pair : Subsystem->PinnedGroups)
    {
        ...
        for (const FPinnedVariable& Pinned : Pair.Value)
        {
            ...
            for (FString Tok : Tokens)
            {
                ...
                {
                    if (IsBPDeclared(Found))
                        B.BPVars.Add(Pinned.VariableName);
                    else if (IsNativeDeclared(Found))
                        B.NativeVars.Add(Pinned.VariableName);
                    else
                        B.BPVars.Add(Pinned.VariableName);
                }
                else
    ...
    auto MakeClassSectionsForPath = [&](const FString& FullPath) -> TSharedRef<SVerticalBox>
    {
        ...
        for (const FName& CN : ClassOrder)
        {
                ...
                if (FStructProperty* SP = CastField<FStructProperty>(P))
                {
                    ...
                    if (SS)
                    {
                        ...
                        if (ValuePtr)
                        {
                            ...
                            SS,
                            reinterpret_cast<uint8*>(ValuePtr));
                ...
                if (IsContainerProperty(P))
                {
                    ...
                    if (!IsComplexStructContainer(P))
                    {
                        ...
                        [VarStr](FName, FName PropName)
                        {
                            ...
                            return N == VarStr || N.StartsWith(VarStr + TEXT(".")) || N.StartsWith(VarStr + TEXT("_"));
                        }));
                    }
                    DV->SetObject(Target);
                    return SNew(SVerticalBox)
            ...
            auto EmitPropWithDelete = [&](UObject* Target, const FName Var, const FName ClassName, const FName CompNameForRemoval) -> TSharedRef<SWidget>
            {
                return SNew(SHorizontalBox)
                       + SHorizontalBox::Slot().FillWidth(1.f)
                         [EmitPropOnly(Target, Var)]
                       + SHorizontalBox::Slot().AutoWidth().VAlign(VAlign_Top).Padding(6, 2, 0, 0)[SNew(SButton).ButtonStyle(FAppStyle::Get(), "FlatButton").ContentPadding(FMargin(4, 2)).ToolTipText(FText::FromString(TEXT("Remove this variable from the list"))).OnClicked(this, &SPinVarPanel::OnRemovePinned, ClassName, Var, FName(*FullPath), CompNameForRemoval)[SNew(STextBlock).Text(FText::FromString(TEXT("X"))).ColorAndOpacity(FLinearColor::Red)]];
            };
    ...
    [&](const FString& ParentPath, const TSharedPtr<FGroupNode>& Node) -> TSharedRef<SWidget>
    {
        ...
        for (const FName& ChildSeg : ChildKeys)
        {
            ...
            TSharedRef<SExpandableArea> ChildArea = SNew(SExpandableArea)
                                                    .InitiallyCollapsed(true)
                                                    .HeaderContent()
...
void SPinVarPanel::ShowAddDialogForDataAsset(UObject* DataAssetInstance)
{
    ...
    auto MakeStringCombo = [](TArray<TSharedPtr<FString>>& Source, TSharedPtr<FString>& Sel)
    {
        ...
        .InitiallySelectedItem(Sel)[SNew(STextBlock).Text_Lambda([&Sel]()
        {
    ...
    // Local BP
    + SVerticalBox::Slot().AutoHeight().Padding(12, 6, 12, 4)[SNew(SVerticalBox).Visibility_Lambda([S]()
    {
        ...
        return bShow ? EVisibility::Visible : EVisibility::Collapsed;
                                                              ...
                                                              + SVerticalBox::Slot().AutoHeight().Padding(0, 2, 0, 0)[MakeStringCombo(S->LocalVarOpts, S->LocalVarSel)]]
    ...
    + SVerticalBox::Slot().AutoHeight().Padding(12, 6, 12, 4)[SNew(SVerticalBox).Visibility_Lambda([S]()
    {
        return (S->SourceType == FState::ESourceType::LocalCppVar)
               ...
               : EVisibility::Collapsed;
                                                              ...
                                                              + SVerticalBox::Slot().AutoHeight().Padding(0, 2, 0, 0)[MakeStringCombo(S->NativePropOpts, S->NativePropSel)]]

    // Group name
    + SVerticalBox::Slot().AutoHeight().Padding(12, 10, 12, 8)[SNew(STextBlock).Text(FText::FromString("Group Name (`|` for subcategories):"))] + SVerticalBox::Slot().AutoHeight().Padding(12, 0, 12, 12)[SNew(SEditableTextBox).Text_Lambda([S]()
    { return FText::FromString(S->GroupStr); }).OnTextChanged_Lambda([S](const FText& T)
    { S->GroupStr = T.ToString(); })]

    // Buttons row
    + SVerticalBox::Slot().AutoHeight().Padding(12, 0, 12, 12)[SNew(SHorizontalBox)

                                                               // Existing group dropdown
                                                               + SHorizontalBox::Slot().AutoWidth().VAlign(VAlign_Center)[SNew(SSearchableComboBox).OptionsSource(&S->ExistingGroupOpts).OnGenerateWidget_Lambda([](TSharedPtr<FString> It)
    {
        return SNew(STextBlock).Text(FText::FromString(It.IsValid() ? *It : TEXT("None")));
    }).OnSelectionChanged_Lambda([S](TSharedPtr<FString> NewSel, ESelectInfo::Type)
    {
        S->ExistingGroupSel = NewSel;
    }).InitiallySelectedItem(S->ExistingGroupSel)[SNew(STextBlock).Text_Lambda([S]()
    {
        return S->ExistingGroupSel.IsValid()
               ...
               : FText::FromString(TEXT("Select group…"));
                                                               ...
                                                               + SHorizontalBox::Slot().AutoWidth().Padding(8, 0, 0, 0).VAlign(VAlign_Center)[SNew(SButton).IsEnabled_Lambda([S]()
    { return S->ExistingGroupSel.IsValid() && !S->ExistingGroupSel->IsEmpty(); }).ButtonStyle(FAppStyle::Get(), "PrimaryButton")
                                                                                                                                              ...
                                                                                                                                              .OnClicked_Lambda([this, S]()
    {
        ...
        const auto GroupName = FName(**S->ExistingGroupSel);
        ...
        return FReply::Handled();
                                                               ...
                                                               // Add from text box
                                                               + SHorizontalBox::Slot().AutoWidth().Padding(0, 0, 8, 0)[SNew(SButton).ButtonStyle(FAppStyle::Get(), "PrimaryButton").Text(FText::FromString("Add")).OnClicked_Lambda([this, S]()
    {
        ...
        Refresh();
        return FReply::Handled();
    })] + SHorizontalBox::Slot().AutoWidth()[SNew(SButton).Text(FText::FromString("Cancel")).OnClicked_Lambda([Dialog]()
    {
...
void SPinVarPanel::ShowAddDialog(UBlueprint* BP)
{
    ...
    if (S->CompOpts.Num())
    {
        ...
        {
            UActorComponent* Template = S->CompSel->Template.IsValid()
            ...
            {
                UE_LOG(LogTemp, Warning, TEXT("PinVar: (init) Template not found for '%s' (key '%s') on %s"), *S->CompSel->Label.ToString(), *S->CompSel->TemplateName.ToString(), *S->Class->GetName());
            }
        }
    }
    ...
    + SVerticalBox::Slot().AutoHeight().Padding(12, 12, 12, 6)
      [SNew(SSegmentedControl<int32>)
       .Value_Lambda([S]()
    { return static_cast<int>(S->SourceType); })
       .OnValueChanged_Lambda([S](int32 NewIdx)
    { S->SourceType = static_cast<FState::ESourceType>(NewIdx); })
       + SSegmentedControl<int32>::Slot(0).Text(FText::FromString("Blueprint local"))
       + SSegmentedControl<int32>::Slot(1).Text(FText::FromString("Parent C++"))
       ...
       .Visibility_Lambda([S]()
    {
       ...
       + SVerticalBox::Slot().AutoHeight().Padding(0, 2, 0, 0)[MakeStringCombo(S->LocalVarOpts, S->LocalVarSel, nullptr)]]
    ...
    + SVerticalBox::Slot().AutoHeight().Padding(12, 6, 12, 4)[SNew(SVerticalBox).Visibility_Lambda([S]()
    {
        return S->SourceType == FState::ESourceType::LocalCppVar
               ...
               : EVisibility::Collapsed;
                                                              ...
                                                              + SVerticalBox::Slot().AutoHeight().Padding(0, 2, 0, 0)[MakeStringCombo(S->NativePropOpts, S->NativePropSel, nullptr)]]
    ...
    + SVerticalBox::Slot().AutoHeight().Padding(12, 6, 12, 4)[SNew(SVerticalBox).Visibility_Lambda([S]()
    {
        ...
        return bShow ? EVisibility::Visible : EVisibility::Collapsed;
                                                              ...
                                                              + SVerticalBox::Slot().AutoHeight()[SNew(STextBlock).Text(FText::FromString("Component:"))] + SVerticalBox::Slot().AutoHeight().Padding(0, 2, 0, 4)[SNew(SSearchableComboBox).OptionsSource(&S->CompOptLabels).OnGenerateWidget_Lambda([](TSharedPtr<FString> It)
    {
        return SNew(STextBlock).Text(FText::FromString(It.IsValid() ? *It : TEXT("None")));
    }).OnSelectionChanged_Lambda([S, this](TSharedPtr<FString> NewSel, ESelectInfo::Type)
    {
        ...
        if (TSharedPtr<FCompOption>* FoundPtr = S->LabelToCompOpt.Find(*NewSel))
        {
            ...
            UActorComponent* Tmpl = S->CompSel->Template.IsValid()
                                    ? S->CompSel->Template.Get()
                                    : Cast<UActorComponent>(FindComponentTemplate(S->Class, S->CompSel->TemplateName));
            ...
            {
                UE_LOG(LogTemp, Warning, TEXT("PinVar: Template not found for '%s' (key '%s') on %s"), *S->CompSel->Label.ToString(), *S->CompSel->TemplateName.ToString(), *S->Class->GetName());
            }
        }
    ...
    })[SNew(STextBlock).Text_Lambda([S]()
    {
        return S->CompSel.IsValid()
               ...
               : FText::FromString(TEXT("None"));
                                                              ...
                                                              + SVerticalBox::Slot().AutoHeight().Padding(0, 4, 0, 0)[SNew(STextBlock).Text(FText::FromString("Property:"))] + SVerticalBox::Slot().AutoHeight().Padding(0, 2, 0, 0)[SAssignNew(S->CompPropCombo, SSearchableComboBox).OptionsSource(&S->CompPropOpts)
    ...
    }).InitiallySelectedItem(S->CompPropSel)[SNew(STextBlock).Text_Lambda([S]()
    {
                                                               ...
                                                               + SHorizontalBox::Slot().AutoWidth().Padding(0, 0, 8, 0)[SNew(SButton).ButtonStyle(FAppStyle::Get(), "PrimaryButton").Text(FText::FromString("Add")).OnClicked_Lambda([this, S]()
    {
        ...
        Refresh();
        return FReply::Handled();
    })] + SHorizontalBox::Slot().AutoWidth()[SNew(SButton).Text(FText::FromString("Cancel")).OnClicked_Lambda([this, Dialog]()
    {
        Dialog->RequestDestroyWindow();
